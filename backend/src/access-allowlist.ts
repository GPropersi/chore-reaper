...
export type RevokeResult =
  { status: 'revoked' } | { status: 'already-absent' } | { status: 'failed'; reason: string };
...
function ruleEmail(rule: unknown): string | undefined {
  if (typeof rule !== 'object' || rule === null) return undefined;
  return (rule as { email?: { email?: string } }).email?.email?.trim().toLowerCase();
}

function includesEmail(include: unknown[], email: string): boolean {
  return include.some((rule) => ruleEmail(rule) === email);
}
...
export async function revokeAccessListEntry(env: AccessEnv, rawEmail: string): Promise<GrantResult> {
  const email = rawEmail.trim().toLowerCase();
  const headers = {
    Authorization: `Bearer ${env.CLOUDFLARE_ACCESS_API_TOKEN}`,
    'Content-Type': 'application/json',
...
// Symmetrical counterpart to grantAccessAndDescribeWarning: best-effort,
// never blocks the D1 delete over an Access-API problem, degrades to a
// warning telling the admin to remove the email manually.
export async function revokeAccessAndDescribeWarning(
  env: AccessEnv,
  email: string,
  logContext: Record<string, unknown>,
): Promise<string | undefined> {
  try {
    const revoke = await revokeAccessListEntry(env, email);
    if (revoke.status === 'failed') {
      console.log(
        JSON.stringify({ event: 'access-revoke-failed', email, reason: revoke.reason, ...logContext }),
      );
      return `User deleted, but could not be removed from the Cloudflare Access allow-list automatically (${revoke.reason}). Remove ${email} manually in the Zero Trust dashboard.`;
    }
    console.log(JSON.stringify({ event: 'access-revoke', email, ...logContext }));
    return undefined;
  } catch (err) {
    console.log(JSON.stringify({ event: 'access-revoke-threw', email, error: String(err), ...logContext }));
    return `User deleted, but could not be removed from the Cloudflare Access allow-list automatically. Remove ${email} manually in the Zero Trust dashboard.`;
  }
}
