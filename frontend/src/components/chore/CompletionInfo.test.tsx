...
describe('CompletionInfo', () => {
  it('formats the completion date in the household timezone, independent of daysSince', () => {
    const date = new Date('2026-01-01T23:30:00.000Z');

    const { rerender } = render(
      <CompletionInfo date={date} daysSince={3} householdTimezone="Pacific/Kiritimati" />,
    );
    // UTC+14: 2026-01-01T23:30Z is already 2026-01-02 locally there.
    expect(screen.getByText(/Jan 2 2026/)).toBeInTheDocument();

    rerender(<CompletionInfo date={date} daysSince={3} householdTimezone="Pacific/Niue" />);
